...
class ActivityC : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val opciones = arrayOf("Homero", "Bart", "Maggie")
        val adapter = ArrayAdapter(this, android.R.layout.simple_spinner_item, opciones)
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        spinner.adapter = adapter

        // Cargar el primer fragmento por defecto
        reemplazarFragment(FragmentHomero())

        // Listener para el spinner
        spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                when (position) {
                    0 -> reemplazarFragment(FragmentHomero())
                    1 -> reemplazarFragment(FragmentBart())
                    2 -> reemplazarFragment(FragmentMaggie())
                }
            }
